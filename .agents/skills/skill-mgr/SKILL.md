...
description: >
  ...
  Use this skill:
    ...
    * When syncing .Codex/skills/ changes to the registry
...
`.Codex/skills/` 관리 체계 안에서 점진적으로 강화합니다.
...
IMPORT    → .Codex/skills/<id>/SKILL.md 로 가져오기
REGISTER  → .Codex/.harness/registries/skills.json 등록
SYNC      → scripts/sync-skills.ps1 실행 → .Codex/skills/ 갱신
...
`.Codex/.harness/registries/skills.json`의 `managedSkills` 배열에 추가:

```json
{
  "id": "<id>",
  "source": ".Codex/skills/<id>/SKILL.md",
  "tier": "optional",
  "mutable": true,
  "tool_aliases": {
    "Codex": ["<id>"]
...
→ `.Codex/skills/<id>/SKILL.md` 갱신 (import 스킬만 해당, 직접 편집 금지)
