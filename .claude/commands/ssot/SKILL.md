---
name: ssot
description: "SSoT/walk 체계로 도메인 스펙을 확인하고 새 feature 작업공간을 만든다. '/ssot', 'ssot 확인', 'walk feature 만들어' 요청 시."
---

# /ssot — 단일 공급 진실원 운영

이 커맨드는 로직을 담지 않는다. **모든 규칙은 공유 규약 한 곳에 있다.**
...
플랫폼 무관 자산은 `docs/`·`scripts/`에만 존재한다. 이 파일은 Claude용 얇은 진입점일 뿐이다.
